...
template <class keyT, class T>
void IndexedMap<keyT, T>::remove(unsigned int index)
{
...
T IndexedMap<keyT, T>::get(const keyT& key) const
{
	...
	throw Exception(exceptionPrefix + "key not found");
}

template <class keyT, class T>
T IndexedMap<keyT, T>::get(unsigned int index) const
{
	if (priv_indexIsValid(index))
		return m_elements[index].value;
	else
		throw Exception(exceptionPrefix + "index out of range");
}
...
template <class keyT, class T>
T& IndexedMap<keyT, T>::access(unsigned int index)
{
	if (priv_indexIsValid(index))
		return m_elements[index].value;
	else
...
template <class keyT, class T>
bool IndexedMap<keyT, T>::valid(unsigned int index) const
{
...
template <class keyT, class T>
void IndexedMap<keyT, T>::set(unsigned int index, const T& value)
{
	if (priv_indexIsValid(index))
...
template <class keyT, class T>
void IndexedMap<keyT, T>::setKey(unsigned int index, const keyT& key)
{
	if (priv_indexIsValid(index))
...
template <class keyT, class T>
keyT IndexedMap<keyT, T>::getKey(unsigned int index) const
{
	if (priv_indexIsValid(index))
...
// PRIVATE
template <class keyT, class T>
bool IndexedMap<keyT, T>::priv_indexIsValid(unsigned int index) const
{
